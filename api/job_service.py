...
from models import CheckJobRequest, CheckJobResponse, SubmitJobRequest, ResultsResponse, ResultsRequest, Response, Instrument, Result
...
class JobService:
    ...
    def results(self, request: ResultsRequest) -> ResultsResponse:
        if request.job_id is not None:
            return ResultsResponse(job_id=request.job_id, responses=[Response(instrument=Instrument(identifier="abc123"), result=Result(source="cache SHOULD MAKE THIS AN ENUM", radar="ABC123"))])
        elif request.group_id is not None:
            return ResultsResponse(group_id=request.group_id, responses=[Response(instrument=Instrument(identifier="abc123"), result=Result(source="cache SHOULD MAKE THIS AN ENUM", radar="ABC123"))])
        else:
            raise ValueError(f"Invalid request. Expected a job_id or a group_id. Received: {request}")
